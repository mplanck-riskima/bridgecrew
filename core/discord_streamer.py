...
class DiscordStreamer:
    ...
    async def start(self, prompt_preview: str = "", persona_name: str = "", session_id: str = "", feature_name: str = "") -> None:
        ...
        debug_parts = []
        if feature_name:
            debug_parts.append(f"`{feature_name}`")
        if session_id:
            debug_parts.append(f"`{session_id[:8]}`")
        debug_suffix = " · " + " · ".join(debug_parts) if debug_parts else ""
        label = f"*{display_name} is thinking...{debug_suffix}*"
        if prompt_preview:
